#!/bin/bash
#
# ECC VS Code Installer
# Installs Everything Claude Code workflows into a VS Code project.
#
# Usage:
#   ./install.sh              # Install to current directory
#   ./install.sh ~            # Install globally to ~/.vscode/
#
...
# into a target project's .vscode/ directory.
find_repo_root() {
    local dir="$(dirname "$SCRIPT_DIR")"
    # First try the parent of SCRIPT_DIR (original layout: .vscode/ lives in repo root)
...
if [ -z "$REPO_ROOT" ]; then
    ...
    echo "This script must be run from within the ECC repository's .vscode/ directory."
    exit 1
fi
...
do_install() {
    ...
    # Check if we're already inside a .vscode directory
    local current_dir_name="$(basename "$target_dir")"
    local vscode_full_path

    if [ "$current_dir_name" = "$VSCODE_DIR" ]; then
        # Already inside the vscode directory, use it directly
        vscode_full_path="$target_dir"
    else
        # Normal case: append VSCODE_DIR to target_dir
        vscode_full_path="$target_dir/$VSCODE_DIR"
    fi

    echo "ECC VS Code Installer"
    echo "====================="
    echo ""
    ...
    # Create the .vscode directory and subdirectories
    mkdir -p "$vscode_full_path"
    SUBDIRS="commands agents skills rules"
    for dir in $SUBDIRS; do
        mkdir -p "$vscode_full_path/$dir"
    done
    ...
    if [ -d "$REPO_ROOT/commands" ]; then
        for f in "$REPO_ROOT/commands"/*.md; do
            ...
            target_path="$vscode_full_path/commands/$local_name"
    ...
    if [ -d "$REPO_ROOT/agents" ]; then
        for f in "$REPO_ROOT/agents"/*.md; do
            [ -f "$f" ] || continue
            local_name=$(basename "$f")
            target_path="$vscode_full_path/agents/$local_name"
    ...
    if [ -d "$REPO_ROOT/skills" ]; then
        for d in "$REPO_ROOT/skills"/*/; do
            ...
            target_skill_dir="$vscode_full_path/skills/$skill_name"
            skill_copied=0
    ...
    if [ -d "$REPO_ROOT/rules" ]; then
        while IFS= read -r rule_file; do
            ...
            target_path="$vscode_full_path/rules/$relative_path"
    ...
    # Copy AGENTS.md (VS Code Claude Code extension reads this)
    if [ -f "$REPO_ROOT/AGENTS.md" ]; then
        if copy_managed_file "$REPO_ROOT/AGENTS.md" "$vscode_full_path/AGENTS.md" "$MANIFEST" "AGENTS.md"; then
            other=$((other + 1))
        fi
    fi

    # Write VS Code settings.json
    SETTINGS_PATH="$vscode_full_path/settings.json"
    SETTINGS_CONTENT='{
  ...
  "claude.agentsFile": "${workspaceFolder}/AGENTS.md"
}'
    if write_managed_file "$SETTINGS_PATH" "$MANIFEST" "settings.json" "$SETTINGS_CONTENT"; then
        other=$((other + 1))
    fi

    # Write VS Code extensions.json
    EXTENSIONS_PATH="$vscode_full_path/extensions.json"
    EXTENSIONS_CONTENT='{
  "recommendations": [
    "anthropic.claude-code"
  ]
}'
    if write_managed_file "$EXTENSIONS_PATH" "$MANIFEST" "extensions.json" "$EXTENSIONS_CONTENT"; then
        other=$((other + 1))
    fi
    ...
    for readme_file in "$SCRIPT_DIR/README.md" "$SCRIPT_DIR/README.zh-CN.md"; do
        if [ -f "$readme_file" ]; then
            ...
            target_path="$vscode_full_path/$local_name"
    ...
    echo "  Skills:    $skills"
    echo "  Rules:     $rules"
    echo "  Other:     $other (settings, extensions, README)"
    echo ""
    echo "Directory:   $VSCODE_DIR"
    echo ""
    echo "Next steps:"
    echo "  1. Open your project in VS Code"
    echo "  2. Install the Claude Code extension (anthropic.claude-code)"
    echo "  3. Type / in the Claude Code panel to see available commands"
    echo "  4. Enjoy the ECC workflows!"
    echo ""
    echo "To uninstall later:"
    echo "  cd $vscode_full_path"
    echo "  ./uninstall.sh"
}
